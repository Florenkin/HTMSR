...
HTMSR 是一个基于 Qt 5.15.2、Visual Studio、CMake、OpenCV、Eigen、PCL 的双目标定与离线点云重建桌面软件工程。当前版本定位为离线处理工具，主要功能包括项目参数配置、双目标定、激光中心线提取、左右图像三维重建、点云显示占位/VTK 适配、点云导出和日志展示。
...
- `PCL`：点云保存和点云可视化适配。
- `Qt5 Widgets / Concurrent`：桌面界面和后台任务。
- `VTK`：可选点云内嵌显示组件。
