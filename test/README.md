...
公共 Debug/Release 预设默认编译测试。在 x64 开发者终端执行 `cmake --workflow --preset verify-release` 或 `verify-debug`，依次完成配置、构建和 CTest。需要 CMake 3.25 或更新版本。
...
重建保存检查使用临时目录和模拟相机验证 `reconstruction/capture` 下的时间戳会话、配对图像及 `reconstruction/result` 下对应的 TXT/PCD；同时验证激光线中文目录、毫秒会话防覆盖、无损左右 PNG、单侧失败清理、红色中心线叠加、空提线仍保存和只读查看器。重新加载点云验证坐标和点数，检查重复重建、单格式重名、并行保存、旧会话兼容、空/失败重建及不可写目录，不改动用户现有采集数据。
重建目录流程检查验证初始左右目录为空、激光线目录和保存开关默认值、关闭路径恢复时不填入旧输入目录但保留激光线输出目录、采集后分别回填、当前选择覆盖先前采集路径、只选择一侧或清空任一侧后禁止重建及任务执行期间锁定路径；使用任意名称且位于不同父目录的两个文件夹生成双目激光帧，用实际重建服务验证所选离线目录能够生成点云并自动保存、重新加载。独立进程验证旧版废弃路径不迁移，新版路径和激光线设置按各自规则恢复，数值参数记忆保持有效。
...
CTest 的 `build_environment` 执行 `HtmsrBuildConfigTests.cmake`，在临时目录模拟不同电脑的依赖路径。检查配置覆盖旧预设、空的 `CMAKE_PREFIX_PATH` 自动生成搜索目录、相对路径和带空格路径、额外前缀列表、依赖变化后缓存失效、相同配置缓存保留、Debug 覆盖顺序及损坏文件保护，不修改实际配置。可直接执行 `cmake -P test/HtmsrBuildConfigTests.cmake`。
...
`CancellationTests.cpp` 随 `serial_diagnostics` 检查算法预取消、等待中断及模拟采集中途取消；硬触发测试另检查等待图像时的取消和线程退出。`config_files` 覆盖便携目录选择、模板初始化和已有文件不覆盖。

`PortablePackageSmoke.ps1 -Executable out/package/HTMSR/htmsr_app.exe` 用仅含 Windows 系统目录的 PATH、无关工作目录和程序相邻配置启动发布包，检查启动日志与正常关闭。此测试使用发布包自己的配置并进行正常设备枚举，不扫描、不发送激光命令。
