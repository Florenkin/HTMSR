...
// 模块概览：2D 和 3D SIFT 特征提取、描述子构建与匹配。
#include "oc_sift.h"
...
namespace opencorr
{
	...
	void SIFT2D::prepare()
	{
		// 算法说明：SIFT 直接操作 OpenCV 图像，因此 prepare 主要是把封装后的输入绑定到特征处理流程中。
	...
	void SIFT2D::compute()
	{
		// 算法说明：2D 路线遵循经典的“检测-描述-匹配”流程，并通过比值测试剔除含糊匹配。
...
#pragma omp section
		...
		// 算法说明：描述子匹配本身是稀疏的，它的任务是提供可靠锚点，而不是直接生成最终的稠密位移场。
...
#pragma omp section
	...
	void SIFT3D::compute()
	{
		// 算法说明：3D SIFT 延续了 2D 的总体思路，但使用的是体数据
		// scale space, volumetric orientation assignment, and 3D descriptors.
		//initialization
		...
		// 算法说明：两个体数据会被独立处理，因此最终
		// correspondences depend on descriptor similarity rather than search bias.
		...
		// 算法说明：这些匹配通常会作为 FeatureAffine3D 或
		// other downstream initialization logic instead of being final outputs.
	...
	{
		// 算法说明：体素间距在物理空间中可能各向异性，因此
		// blur kernel is adjusted per axis to preserve scale meaning.
...
#pragma omp section
	...
	// Implements a reusable ordering rule used to rank candidate results.
