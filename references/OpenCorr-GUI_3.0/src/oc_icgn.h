...
namespace opencorr
{
	...
	// 算法说明：ICGN 是 OpenCorr 中最核心的高精度求解器之一。
	// 算法说明：它先预计算参考侧量，再不断迭代变形目标子区，直到变形增量足够小。
	class ICGN2D1_
	{
	public:
	...
	// 算法说明：二阶模型扩展了局部变形场，使求解器能够表示更强的曲率和旋转效应。
	// 算法说明：这会提升模型灵活性，但也会增加参数数量和计算代价。
	class ICGN2D2_
	{
	public:
		std::unique_ptr<Subset2D> ref_subset;
	...
	// 算法说明：3D 求解器把同样的反向组合思想扩展到了体数据。
	// 算法说明：它的核心循环与 2D 版本结构相似，但使用的是 3D 梯度、3D 插值和 12 个运动参数。
	class ICGN3D1_
	{
	public:
