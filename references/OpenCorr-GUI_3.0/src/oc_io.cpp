...
namespace opencorr
{
	// 构造 IO2D 对象，并初始化后续步骤需要的状态。
	IO2D::IO2D() {}

	// 释放 IO2D 持有的缓冲区、计划或动态资源。
	IO2D::~IO2D() {}
	...
	{
		// 算法说明：很多流程会在外部定义 POI，因此轻量级 CSV 点导入是应用层中的一等功能。
	...
	{
		// 算法说明：双目标定必须成对读入两台相机的数据，
		// 因为匹配与重建过程始终同时依赖左右两个视角。
		std::ifstream file_in(file_path);
		if (!file_in.is_open())
		{
			std::cerr << "failed to read file " << file_path << std::endl;
	...
	{
		// 算法说明：持久化保存的 POI 表允许后续继续做应变分析、可视化
		// 或求解器对比，而不必重新执行整套 DIC 计算。
		std::ifstream file_in(file_path);
		if (!file_in.is_open())
		{
			std::cerr << "failed to read file " << file_path << std::endl;
	...
	{
		// 算法说明：结果图导出会把 POI 数值重新映射回图像网格，因此场结果可以直接在外部绘图工具中可视化。
	...
	{
		// 算法说明：双目 POI 记录同时包含 2D 图像测量信息与 3D 重建上下文，
		// 因此这里会把它作为一个复合对象整体加载。
		std::ifstream file_in(file_path);
		if (!file_in.is_open())
		{
	...
	{
		// 算法说明：双目导出会把多视图匹配质量、图像坐标、
		// 三维坐标和应变统一保存在一张自包含的数据表中。
		std::ofstream file_out(file_path);
		file_out.setf(std::ios::fixed);
		// 更新当前对象使用的配置、输入或运行时状态。
		file_out << std::setprecision(8);
	...
	{
		// 算法说明：即使变量来自双目推导，它也会被重新映射回左视图网格，
		// 以便统一做场可视化和诊断分析。
		int height = getHeight();
		int width = getWidth();
		Eigen::MatrixXf output_map = Eigen::MatrixXf::Zero(height, width);
	...
	// 构造 IO3D 对象，并初始化后续步骤需要的状态。
	IO3D::IO3D() {}

	// 释放 IO3D 持有的缓冲区、计划或动态资源。
	IO3D::~IO3D() {}
	...
	{
		// 算法说明：3D 结果表是一次体相关求解的紧凑摘要，
		// 其中同时包含位移、位移梯度、应变和质量信息。
		std::ofstream file_out(file_path);
		file_out.setf(std::ios::fixed);
		// 更新当前对象使用的配置、输入或运行时状态。
		file_out << std::setprecision(8);
	...
	{
		// 算法说明：3D 结果图会把采样场重新序列化成类似体数据的布局，
		// 方便下游工具继续切片、可视化或后处理。
	...
	{
		// 算法说明：重新加载二进制矩阵可以快速恢复关键 DVC 结果字段，
		// 同时避免文本解析带来的额外开销。
		std::ifstream file_in(file_path);
		if (!file_in.is_open())
		{
