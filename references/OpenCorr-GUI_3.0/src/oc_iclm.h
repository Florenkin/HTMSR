...
namespace opencorr
{
	...
	{
		// 算法说明： lambda controls the damping strength, while alpha and beta
		// shrink or enlarge the damping after each trial step.
		float lambda = 100.f;
		float alpha = 0.1f;
		float beta = 10.f;
	};

	// Per-thread cache block for the 2D first-order ICLM solver.
	class ICLM2D1_
	{
	public:
	...
	{
	private:
		// 算法说明： ICLM keeps the inverse-compositional structure of ICGN,
		// but augments the Hessian with a damping term to stabilize difficult updates.
	...
	// Per-thread cache block for the 2D second-order ICLM solver.
	class ICLM2D2_
	{
	public:
		std::unique_ptr<Subset2D> ref_subset;
	...
	{
	private:
		// 算法说明： the second-order model expands the warp parameterization,
		// and the LM damping plays the same role as in the first-order solver.
