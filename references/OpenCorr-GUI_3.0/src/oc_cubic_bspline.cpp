...
namespace opencorr
{
	...
	// 释放 BicubicBspline 持有的缓冲区、计划或动态资源。
	...
	{
		// 算法说明：每个整数像素单元都会得到自己的局部多项式块，
		// 因而后续 `compute()` 只需要做一次局部求值。
		if (coefficient != nullptr)
		{
			delete4D(coefficient);
		}
...
#pragma omp parallel for
		for (int r = 1; r < height - 2; r++)
		{
			for (int c = 1; c < width - 2; c++)
			{
				...
				// 算法说明：灰度邻域会在这里一次性转换成样条系数，
				// 这样后续查询时就不必反复从原始像素重新推导。
				// 计算插值系数矩阵。
				float mat_p[4][4] = { 0.f };
				for (int k = 0; k < 4; k++)
				{
	...
	// 释放 TricubicBspline 持有的缓冲区、计划或动态资源。
	...
	{
		// 算法说明：3D 样条系数通过沿 x、y、z 三轴的可分离滤波得到，
		// 这比做一次完整的稠密三维求解便宜得多。
		if (coefficient != nullptr)
		{
			delete3D(coefficient);
		...
		// 算法说明：由于三次样条预滤波是可分离的，所以每个方向都能独立处理，
		// 这让实现在线性扩展到 3D 时依然可控。
		// 沿 x 轴卷积。
#pragma omp parallel for
