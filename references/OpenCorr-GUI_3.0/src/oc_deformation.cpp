...
namespace opencorr
{
	// 构造 Deformation2D1 对象，并初始化后续步骤需要的状态。
	...
	// 释放 Deformation2D1 持有的缓冲区、计划或动态资源。
	Deformation2D1::~Deformation2D1() {}
	...
	{
		// 算法说明：迭代求解器会在变形后的局部坐标上采样目标图像，因此这个映射函数是整个库里最热的路径之一。
	...
	{
		// 算法说明：有些求解器会先直接更新 warp 矩阵，再恢复显式参数形式用于存储和输出。
		u = warp_matrix(0, 2);
	...
	{
		// 算法说明：同时保存参数和齐次 warp 矩阵，可以让迭代更新公式中的组合和点映射更简单。
	...
	// 构造 Deformation2D2 对象，并初始化后续步骤需要的状态。
	...
	// 释放 Deformation2D2 持有的缓冲区、计划或动态资源。
	Deformation2D2::~Deformation2D2() {}
	...
	{
		// 算法说明：二次基函数会先展开局部坐标，再施加 warp，这正是二阶形变的表达方式。
	...
	{
		// 算法说明：矩阵以紧凑形式存储二次项系数，因此在转换回参数记号时有些项需要缩放。
		u = warp_matrix(3, 5);
	...
	// 构造 Deformation3D1 对象，并初始化后续步骤需要的状态。
	...
	// 释放 Deformation3D1 持有的缓冲区、计划或动态资源。
	Deformation3D1::~Deformation3D1() {}
