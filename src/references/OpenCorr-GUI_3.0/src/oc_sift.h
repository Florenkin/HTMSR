...
namespace opencorr
{
	...
	// 2D SIFT 特征提取、描述子构建与匹配器。
	class SIFT2D : public Feature2D
	{
	protected:
		// 算法说明： 2D SIFT mainly provides robust sparse seeds for later dense
		// refinement under large displacement, rotation, or weak initial guesses.
	...
	// 将 Eigen 矩阵计算接入 3D SIFT 流程的辅助结构。
	struct EigenMatrix
	{
		float eigen_value;
	...
	// 3D SIFT 特征提取、描述子构建与匹配器。
	class SIFT3D : public Feature3D
	{
	private:
		// 算法说明： orientation quantization on an icosahedron gives a more
		// uniform directional partition for 3D descriptors.
		TriangleTile icosahedron[20]; //triangles in an icosahedron

	protected:
		// 算法说明： 3D SIFT is valuable in DVC because sparse landmark matches
		// can rescue cases where brute-force dense initialization is unreliable.
