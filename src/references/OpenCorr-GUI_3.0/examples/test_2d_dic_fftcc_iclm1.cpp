/*
 ...
 DIC method based on FFT-CC algorithm and IC-LM algorithm (with the 1st order
 shape function).
*/
...
// 模块概览：示例，组合 FFTCC 和一阶 ICLM，实现 2D DIC。
#include "opencorr.h"
...
int main()
{
	...
	ICLM2D1* iclm1 = new ICLM2D1(subset_radius_x, subset_radius_y, max_deformation_norm, max_iteration, cpu_thread_number);
	iclm1->setImages(ref_img, tar_img);
	iclm1->setDamping(lambda, alpha, beta);
	iclm1->prepare();
	iclm1->compute(poi_queue);
	...
	cout << "Deformation determination using ICLM takes " << consumed_time << " sec." << std::endl;

	//save the calculated dispalcements
	file_path = tar_image_path.substr(0, tar_image_path.find_last_of(".")) + "_fftcc_iclm1_r16.csv";
	in_out.setPath(file_path);
	in_out.saveTable2D(poi_queue);

	//save the computation time
	file_path = tar_image_path.substr(0, tar_image_path.find_last_of(".")) + "_fftcc_iclm1_r16_time.csv";
	csv_out.open(file_path);
	if (csv_out.is_open())
	{
		csv_out << "POI number" << delimiter << "Initialization" << delimiter << "FFTCC" << delimiter << "ICLM" << endl;
